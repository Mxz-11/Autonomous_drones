...
def frame_rgb_to_jpeg_bytes(frame_rgb: np.ndarray, *, max_edge: int = DEFAULT_MAX_EDGE, quality: int = DEFAULT_JPEG_QUALITY) -> bytes:
    ...
    buf = io.BytesIO()
    im.save(buf, format="JPEG", quality=int(max(1, min(quality, 95))), optimize=True, subsampling=2)
    return buf.getvalue()
...
def encode_frame_for_vlm(frame_rgb: np.ndarray, *, profile: Profile = "default") -> tuple[str, str]:
    if profile == "tiny":
        raw = frame_rgb_to_jpeg_bytes(frame_rgb, max_edge=TINY_MAX_EDGE, quality=TINY_JPEG_QUALITY)

    else:
        raw = frame_rgb_to_jpeg_bytes(frame_rgb, max_edge=DEFAULT_MAX_EDGE, quality=DEFAULT_JPEG_QUALITY)
...
def encode_single(frame_rgb: np.ndarray, *, max_edge: int = 256, quality: int = 70) -> str:
    raw = frame_rgb_to_jpeg_bytes(frame_rgb, max_edge=max_edge, quality=quality)
    return jpeg_bytes_to_b64(raw)


_CTX_ERR_RE = re.compile(r"context|token|length|maximum|too long|window|exceed", re.IGNORECASE)
