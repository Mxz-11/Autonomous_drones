...
LMSTUDIO_OPENAI_BASE = os.environ.get("LMSTUDIO_OPENAI_BASE", "http://127.0.0.1:1235/v1",)
...
VLM_MODELS: dict[str, str] = {
    ...
    "gemma-4b":     "google/gemma-4-e4b",
}

ACTIVE_VLM_MODEL = "smolvlm2"
...
ACTIVE_SUMMARY_MODEL = ACTIVE_VLM_MODEL
...
def get_decision_llm() -> LMStudioCompatChatOpenAI:
    return _base_chat_openai(DECISION_MODEL_ID, DECISION_TEMPERATURE, DECISION_MAX_TOKENS, DECISION_TIMEOUT_S)


def get_summary_llm() -> LMStudioCompatChatOpenAI:
    return _base_chat_openai(SUMMARY_MODEL_ID, SUMMARY_TEMPERATURE, SUMMARY_MAX_TOKENS, SUMMARY_TIMEOUT_S)


def get_custom_llm(model_id: str, temperature: float = 0.3, max_tokens: int = 256, base_url: str | None = None) -> LMStudioCompatChatOpenAI:
    kwargs: dict[str, Any] = {"model": model_id, "temperature": temperature, "max_tokens": max_tokens, "timeout": DECISION_TIMEOUT_S, "base_url": base_url or LMSTUDIO_OPENAI_BASE, "api_key": OPENAI_API_KEY, "max_retries": 1}
    try:
        return LMStudioCompatChatOpenAI(**kwargs)
...
class LMStudioChat(BaseChatModel):
    ...
    def _build_payload(self, messages: list[BaseMessage]) -> dict:
        ...
        payload: dict[str, Any] = {"model": self.model_name, "input": input_text, "temperature": self.temperature, "max_tokens": self.max_tokens}
        if system_prompt:
    ...
    def _generate(self, messages: list[BaseMessage], stop: list[str] | None = None, run_manager: CallbackManagerForLLMRun | None = None, **kwargs: Any) -> ChatResult:
        ...
        if not usage_raw:
            ...
            usage_raw = {"prompt_tokens": stats.get("input_tokens", 0), "completion_tokens": stats.get("total_output_tokens", 0), "total_tokens": stats.get("input_tokens", 0) + stats.get("total_output_tokens", 0)}

        usage_metadata = {"prompt_tokens": usage_raw.get("prompt_tokens", 0), "completion_tokens": usage_raw.get("completion_tokens", 0), "total_tokens": usage_raw.get("total_tokens", 0)}
        ...
        response_metadata = {"model_name": self.model_name, "finish_reason": data.get("finish_reason", "stop"), "token_usage": usage_metadata}
        usage_meta = {"input_tokens": usage_metadata["prompt_tokens"], "output_tokens": usage_metadata["completion_tokens"], "total_tokens": usage_metadata["total_tokens"], "prompt_tokens": usage_metadata["prompt_tokens"], "completion_tokens": usage_metadata["completion_tokens"]}
        ai_message = AIMessage(content=content, response_metadata=response_metadata, usage_metadata=usage_meta)
